...
/**
 * WebSocket client wrapper for cottage 一起看 (watch-together).
 *
 * Mirrors cottageListenWs.js: auto-reconnect with exponential backoff,
 * app-level PING/PONG keepalive, and inbound PONG handled internally while
 * everything else is forwarded to onEvent. Only the endpoint path differs.
 */
...
function deriveWsUrl() {
  const path = "/v1/cottage/watch/ws";
...
export function createWatchSocket({ onEvent, onOpen, onClose }) {
  let ws = null;
  let pingTimer = null;
  ...
  function open() {
    ...
    ws.addEventListener("message", (event) => {
      ...
      try {
        ...
        console.error("[watch-ws] onEvent threw:", e);
      }
    });
