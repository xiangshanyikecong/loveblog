  ...
  <div class="content-section ttt-view">
    <article class="glass-card section-block ttt-head">
      <div class="head-row">
        ...
        <h2>{{ t("cottageGames.linklink.title") }}</h2>
      ...
      <div class="players">
        <div class="player-card" :class="{ active: isPlaying && room.turn_uid === myUid }">
          ...
          <div class="player-meta">
            ...
            <p class="player-sub">{{ colorLabel(myColor) }} · {{ t("cottageGames.linklink.score") }} {{ myScore }} · {{ t("cottageGames.common.winLabel") }} {{ myWins }}</p>
          </div>
        </div>
        <div class="vs">VS</div>
        <div class="player-card" :class="{ active: isPlaying && room.turn_uid === partnerUid }">
          ...
          <div class="player-meta">
            ...
            <p class="player-sub">{{ colorLabel(partnerColor) }} · {{ t("cottageGames.linklink.score") }} {{ partnerScore }} · {{ t("cottageGames.common.winLabel") }} {{ partnerWins }}</p>
          </div>
        </div>
      </div>
...
import LinkLinkBoard from "./LinkLinkBoard.vue";
...
const statusText = computed(() => {
  if (room.phase === "waiting") return t("cottageGames.linklink.waitingHint");
  if (room.phase === "finished") {
    if (room.winner === "draw") return t("cottageGames.linklink.drawStatus", { score: scoreLine.value });
    const winnerUid = room.winner === "black" ? room.black_uid : room.white_uid;
    return winnerUid === myUid.value
      ? t("cottageGames.linklink.winStatus", { score: scoreLine.value })
      : t("cottageGames.linklink.loseStatus", { name: partnerName.value || t("cottageGames.common.partner"), score: scoreLine.value });
  }
  if (isMyTurn.value) {
    return t("cottageGames.linklink.yourTurn", { mark: markLabel(myColor.value), score: scoreLine.value });
  }
  return t("cottageGames.linklink.waitingPartner", { score: scoreLine.value });
});
...
function colorLabel(color) {
  if (color === "black") return t("cottageGames.linklink.player1");
  if (color === "white") return t("cottageGames.linklink.player2");
...
// (and the wire adds game_key / server_ts_ms / size); blindly Object.assign-ing
// them pollutes the gameplay state object.
const ROOM_FIELDS = [
  "phase", "cols", "rows", "cells", "turn", "turn_uid", "winner", "win_line",
  "last_move", "move_count", "black_uid", "white_uid", "black_score",
  "white_score", "pending", "undo_request_by", "seq"
];
...
function handleEvent(e) {
  ...
  if (e.type === "STATE") {
    ...
    showMessage(t("cottageGames.linklink.inviteToast", { name: from }));
...
async function invite() {
  try {
    ...
    showMessage(t("cottageGames.linklink.inviteCalledToast"));
  } catch (error) {
