...
<template>
  <div class="content-section gomoku-view">
    <article class="glass-card section-block gomoku-head">
      <div class="head-row">
        <router-link to="/cottage/games" class="back-link">{{ t("cottageGames.common.backToGames") }}</router-link>
        <h2>{{ t("cottageGames.gomoku.title") }}</h2>
      ...
      <div class="players">
        <div class="player-card" :class="{ active: isPlaying && room.turn_uid === myUid }">
          ...
          <div class="player-meta">
            ...
            <p class="player-sub">{{ colorLabel(myColor) }} · {{ t("cottageGames.common.winLabel") }} {{ myWins }}</p>
          </div>
        </div>
        <div class="vs">VS</div>
        <div class="player-card" :class="{ active: isPlaying && room.turn_uid === partnerUid }">
          ...
          <div class="player-meta">
            ...
            <p class="player-sub">{{ colorLabel(partnerColor) }} · {{ t("cottageGames.common.winLabel") }} {{ partnerWins }}</p>
          </div>
        </div>
      </div>
...
import { fetchGomokuMatches, fetchGomokuState, fetchMe, inviteGomokuPartner } from "../../../../lib/api";
...
const partnerUid = computed(() => {
  ...
  // Fall back to any known partner name key that isn't me.
...
const statusText = computed(() => {
  ...
  if (room.phase === "finished") {
    if (room.winner === "draw") return t("cottageGames.common.drawRematch");
    const winnerUid = room.winner === "black" ? room.black_uid : room.white_uid;
    return winnerUid === myUid.value ? t("cottageGames.common.youWinCelebration") : t("cottageGames.common.partnerWinRematch", { name: partnerName.value || t("cottageGames.common.partner") });
  }
  return isMyTurn.value ? t("cottageGames.common.yourTurn", { mark: colorLabel(myColor.value) }) : t("cottageGames.common.waitingPartnerMove");
});
...
function colorLabel(color) {
  if (color === "black") return t("cottageGames.gomoku.black");
  if (color === "white") return t("cottageGames.gomoku.white");
...
function applyState(payload) {
  if (!payload) return;
  // Ignore stale frames (snapshots carry a monotonic seq).
...
async function refreshStats() {
  try {
    const data = await fetchGomokuMatches(1);
...
function handleEvent(e) {
  ...
  if (e.type === "STATE") {
    ...
    showMessage(t("cottageGames.gomoku.inviteToast", { name: from }));
...
onMounted(async () => {
  ...
  try {
    const state = await fetchGomokuState();
...
<style scoped>
.gomoku-view { display: grid; gap: 1rem; }
.gomoku-head { padding: 1.1rem 1.2rem; display: grid; gap: 0.9rem; }
.head-row {
  display: flex;
  align-items: center;
