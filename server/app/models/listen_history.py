...
class ListenHistoryEntry(Base):
    """A song played in cottage listen-together (shared couple history)."""

    __tablename__ = "listen_history"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    hid: Mapped[str] = mapped_column(
        String(36), default=lambda: str(uuid.uuid4()), unique=True, nullable=False, index=True
    )
    song_id: Mapped[str] = mapped_column(String(32), nullable=False, index=True)
    ...
    started_by_uid: Mapped[str] = mapped_column(String(36), nullable=False, index=True)
